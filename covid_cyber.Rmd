...
p +  scale_fill_manual(values = alpha(c("white", "blue") , 0.6) , labels= c("NO", "YES"))+
...
# p0 <- ggplot(data = ui_state_policy ,
#              mapping = aes(x = month, y = initclaims_rate_regular, color = factor(stayathome)) )


#p1 <- p0 + geom_line(color = "gray70", mapping = aes(group = state))


p2 <- p1 + geom_smooth(mapping = aes(group = stayathome), size= 1.5, se = TRUE) + scale_color_manual(values=c("#E69F00", "#56B4E9"))

p3 <- p2 + geom_text_repel(data = subset(ui_state_policy, month == max(month) ), 
                           mapping = aes(x = month, y = initclaims_rate_regular, label = state),
                           size = 1.8, segment.color = NA, nudge_x = 30) +
  coord_cartesian(c(min(ui_state_policy$month),
                    max(ui_state_policy$month)))


p3 + labs(x = "", y = "Unemployment Rate", title = "State-Level Unemployment Rate, 2021")

# +
#   facet_wrap(~ reorder(stayathome, -initclaims_rate_regular, na.rm = TRUE), nrow = 1)
