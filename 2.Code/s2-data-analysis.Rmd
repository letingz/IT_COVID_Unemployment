...
col <-  c('SITEID', 'EMPLE','REVEN','SICGRP', 'SICSUBGROUP')
ci_site <- fread(path, select = col)


psych::describe(ci_site[, c('EMPLE', 'REVEN')])

ci_site <- ci_site %>% 
  mutate(size_group = case_when(
      between(EMPLE, 0, 50) ~ "0 to 50",
      between(EMPLE, 50, 100) ~ "50 to 100",
      between(EMPLE, 100, 500) ~ "100 to 500",
      between(EMPLE, 500, 1000) ~ "500 to 1000",
      EMPLE> 1000 ~ ">1000"
      
  ),
       rev_group = case_when(
         between(REVEN, 0, 5) ~ "0 to 5",
         between(REVEN, 5, 10 ) ~ "5 to 10",
         between(REVEN, 10, 50 ) ~ "10 to 50",
         between(REVEN, 50, 100 ) ~ "50 to 100",
         REVEN>100 ~ "> 100"))

```


```{r CI Summary statistics: Revenue, Size}
library(ggpubr)


positions <- c("0 to 50", "50 to 100", "100 to 500", "500 to 1000", ">1000")

ci_site %>% 
  group_by(size_group) %>% 
  summarise(counts = n()) %>% 
  arrange(desc(counts)) %>% 
  ggplot(aes(x = size_group, y = counts)) +
  scale_x_discrete(limits = positions) +
  geom_bar(stat = "identity")  + 
  theme_pubclean()


positions <- c("0 to 5", "5 to 10", "10 to 50", "50 to 100", ">100")

ci_site %>% 
  group_by(rev_group) %>% 
  summarise(counts = n()) %>% 
  arrange(desc(counts)) %>% 
  ggplot(aes(x = rev_group, y = counts)) +
  scale_x_discrete(limits = positions) +
  geom_bar(stat = "identity")  + 
  theme_pubclean()

```
```{r CI Summary statistics : Industries}

ci_site %>% 
  group_by(SICGRP) %>% 
  summarise(counts = n()) %>% 
  arrange(desc(counts)) %>% 
  ggplot(aes(x = SICGRP, y = counts)) +
 # scale_x_discrete(limits = positions) +
  geom_bar(stat = "identity")  + 
  coord_flip()+
  theme_pubclean()
...
ci_site %>% 
  group_by(SICSUBGROUP) %>% 
  summarise(counts = n()) %>% 
  arrange(desc(counts)) %>% 
  ggplot(aes(x = reorder(SICSUBGROUP, counts), y = counts )) +
 # scale_x_discrete(limits = positions) +
  geom_bar(stat = "identity")  + 
  coord_flip()+
  theme_pubclean()
