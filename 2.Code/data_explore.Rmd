...
ui_state_policy <- merge(ui_mean_state, state_home, by = "statefips", all.x = TRUE )
 ...
 ggplot(data = ui_state_policy ,
     ...
     scale_color_manual(values=c("#E69F00", "#56B4E9")) +
      ...
      labs(x = "", y = "Unemployment Rate", title = "State-Level Unemployment Rate, 2021")
...
#   group_by(month, home) %>% 
#   summarise(emp = sum(emp_combined, na.rm = TRUE)) %>% 
#   ggplot(aes(x = month, y = emp, color = factor(home))) +
#   scale_x_continuous(breaks = seq(1,10,1), labels = function(x)month.abb[x]) +  geom_line()
...
panel_use1 %>% 
  ...
  summarise(emp = sum(emp_combined, na.rm = TRUE),
   ...
   filter(month<10) %>% 
  ggplot(aes(x = month, y = value, color = group)) +
  scale_x_continuous(breaks = seq(1,10,1), labels = function(x)month.abb[x])+
  geom_line()
```
...
panel_use1 %>% 
  ...
  ggplot(mapping =aes(x = month, y = emp, color = factor(home))) +  scale_x_continuous(breaks = seq(1,10,1), labels = function(x)month.abb[x])+geom_line()

'```

```{r Correlation}
panel_use1 %>% 
  group_by(stateabbrev, home) %>% 
  summarise(emp = 100* mean(emp_combined, na.rm = TRUE),
            unemp = 100* mean(avg_initclaims_rate, na.rm = TRUE),
            its_emp = mean(its_emps_all, na.rm = TRUE),
            all_emp = mean(all_earn_all, na.rm = TRUE),
            its_rate = log(its_emp*100/all_emp+1)) %>% 
  ggplot(aes(x = its_rate, y = unemp, color = factor(home))) + geom_point()
...
ci_data_use  %>% select(SITEID, COUNTY, EMPLE, REVEN,
                          IT_BUDGET, HARDWARE_BUDGET, 
                       SOFTWARE_BUDGET,SERVICES_BUDGET) %>% 
  filter(!is.na(COUNTY) & IT_BUDGET != 0 & EMPLE!=0) %>%
  filter(COUNTY %in% c(72091, 72135, 72137, 72145, 486301, 15005)) %>% 
  group_by(COUNTY) %>% 
...
ci_data_use  %>% select(SITEID, COUNTY, EMPLE, REVEN,
  ...
  filter(!is.na(COUNTY) & IT_BUDGET != 0 & EMPLE!=0) %>%
  group_by(COUNTY) %>% 
  mutate(count_county_estab, 
         it_budget_per_emp = IT_BUDGET/EMPLE,
         emple_win = winsorize(EMPLE[count_county_estab>1]), 
