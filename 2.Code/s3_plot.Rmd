...
county_it <- county_week %>% select(county, q4_high_it_median,q2_high_lowskill, q4_high_lowskill) %>% distinct()%>%mutate(countyfips = as.integer(county))
...
skill_group_county_unemployrate <- county_use %>% 
  mutate(value = as.numeric(value)) %>% 
  group_by(q2_high_lowskill, date) %>% 
  summarise(unemployment = mean(value, na.rm = TRUE)) %>% 
  ungroup()
```
...
skill_group_county_unemployrate %>% filter(!is.na(q2_high_lowskill)) %>%
  
  ggplot(aes(x = date, y = unemployment, 
            color = factor(q2_high_lowskill) )) +
  geom_line(size= 1.2) +
  scale_color_manual(name = "Low Skill", labels = c("YES", "NO"),values=c( "blue", "red"))+
